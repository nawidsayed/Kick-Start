...
long solve(long t){
    ...
    while(swapped){
        swapped = false;
        FOR(n,1,N){
            if (mat[n][0] * mat[n-1][2]  <  mat[n][2] * mat[n-1][0]){
                swap(mat[n-1], mat[n]);
                swapped = true;
            }
        }
    ...
    long total_time = 1;
    FOR(n,0,N) total_time += mat[n][0];

    func = ml(total_time,vl(N+1,0));
    
    FORR(n,0,N){
        vl vec = mat[n];
        FOR(time,0,total_time - vec[0]){
            long available_e = max(vec[1] - time*vec[2], 0L);
            long reduced_e = func[time+vec[0]][n+1];
            long cur_best_e = func[time][n+1];
            // print(n, time, available_e, reduced_e, cur_best_e, vec[0])
            func[time][n] = max(available_e + reduced_e, cur_best_e);
        }
    }

    cout << "Case #" << t+1 << ": " << func[0][0] << endl;
